...
// /**
//  * postStatus — accepts a pre-built FormData directly from StatusUploader.
//  *
//  * WHY FormData passthrough (not object destructuring):
//  *   useStatus.handlePost receives FormData from StatusUploader and calls
//  *   postStatus(formData). If we destructure as { text, file, ... }, every
//  *   value is undefined because FormData is not a plain object — you cannot
//  *   destructure it. That means form.append() gets undefined for every field,
//  *   the backend receives an empty body, and returns:
//  *   "Status must have text or a media file".
//  *
//  * WHY no manual Content-Type header:
//  *   axios auto-detects FormData and sets multipart/form-data WITH the correct
//  *   boundary string. If you set Content-Type manually, the boundary is missing,
//  *   multer cannot parse the body, req.body and req.file are both undefined,
//  *   same error as above.
//  *
//  * @param {FormData} formData - built by StatusUploader with fields:
//  *   text? (string), file? (File), backgroundColor, textColor, privacy
//  */
// export async function postStatus(formData) {
//   // Do NOT pass headers — axios sets multipart + boundary automatically
...
// /**
//  * replyToStatus — stub until backend endpoint exists.
//  * Implement: POST /api/statuses/:statusId/replies
//  */
