...
function socketHandler(io) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", ({ receiverId, text }, callback) => {
      try {
        if (!text?.trim())
          return callback?.({ ok: false, error: "Message required" });
        console.log("azx ", text.length);
        if (text.length > 22000)
          return callback?.({
            ok: false,
            error: "Message too long (max 2000 chars)",
          });
        if (!receiverId)
          return callback?.({ ok: false, error: "receiverId required" });
    ...
    socket.on("send_group_message", ({ groupId, text }, callback) => {
      try {
        ...
        if (!groupId)
          return callback?.({ ok: false, error: "groupId required" });
    ...
    socket.on("edit_message", ({ messageId, text }, callback) => {
      try {
        if (!text?.trim())
          return callback?.({ ok: false, error: "Text required" });

        const db = readDB();
        const msgIdx = db.messages.findIndex((m) => m.id === messageId);

        if (msgIdx === -1)
          return callback?.({ ok: false, error: "Message not found" });
        const msg = db.messages[msgIdx];
        if (msg.senderId !== currentUserId) {
          return callback?.({
            ok: false,
            error: "Cannot edit others' messages",
          });
        }
        if (msg.deletedAt)
          return callback?.({ ok: false, error: "Message was deleted" });
      ...
      try {
        ...
        if (deleteFor === "everyone") {
          ...
          if (msg.chatType === "group") {
            ...
            io.to(msg.receiverId).emit("message_deleted", {
              messageId,
              deletedFor: "everyone",
            });
          }
    ...
    socket.on("typing_start", ({ receiverId, groupId }) => {
      if (groupId) {
        socket
          .to(`group:${groupId}`)
          .emit("typing_start", { userId: currentUserId, groupId });
      } else if (receiverId) {
    ...
    socket.on("typing_stop", ({ receiverId, groupId }) => {
      if (groupId) {
        socket
          .to(`group:${groupId}`)
          .emit("typing_stop", { userId: currentUserId, groupId });
      } else if (receiverId) {
